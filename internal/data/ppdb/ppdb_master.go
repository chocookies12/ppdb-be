...
func (d Data) GetLoginAdmin(ctx context.Context, login ppdbEntity.TableAdmin) (ppdbEntity.TableAdmin, error) {
	var (
		err    error
		result ppdbEntity.TableAdmin
	)

	err = (*d.stmt)[loginAdmin].QueryRowxContext(ctx, login.EmailAdmin).StructScan(&result)
	if err != nil {
		return result, errors.Wrap(err, "[DATA][GetLoginAdmin]")
	}
...
func (d Data) GetPembayaranFormulirDetail(ctx context.Context, idpesertadidik string) (ppdbEntity.TablePembayaranFormulir, error) {
	...
	err := (*d.stmt)[getPembayaranFormulirDetail].QueryRowxContext(ctx, idpesertadidik).Scan(
		...
		&pembayaranformulir.EmailPeserta,
		&pembayaranformulir.StatusID,
